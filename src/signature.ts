...
export function sign(message: Uint8Array, secret: bigint, sha256: (_: Uint8Array) => Uint8Array) {
    const m = message
    const d = secret
    ...
    const k0 = int(sha256(concat(bufferFromBigInt(d), m))) % curve.n
    if (k0 === 0n) {
        throw new Error('sig failed')
    }
    const R = ec.multiply(curve.g, k0)
    const k = jacobi(R.y) === 1n ? k0 : curve.n - k0
    const e = int(sha256(concat(bufferFromBigInt(R.x), pointToBuffer(ec.multiply(curve.g, d)), m))) % curve.n
...
export function verify(
    ...
    message: Uint8Array,
    sig: Uint8Array,
    sha256: (_: Uint8Array) => Uint8Array
): boolean {
    ...
    const e = int(sha256(concat(bufferFromBigInt(r), pointToBuffer(P), m))) % curve.n
